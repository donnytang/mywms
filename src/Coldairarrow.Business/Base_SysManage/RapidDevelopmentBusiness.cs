...
namespace Coldairarrow.Business.Base_SysManage
{
    public class RapidDevelopmentBusiness : BaseBusiness<Base_DatabaseLink>
    {
        ...
        /// <returns></returns>
        public List<DbTableInfo> GetDbTableList(int linkId)
        {
        ...
        public void BuildCode(int linkId, string areaName, string tables, string buildType)
        {
            //内部成员初始化
            ...
            tableList.ForEach(aTable =>
            {
                ...
                var keyField = tableFieldInfo.Where(i => i.IsKey).FirstOrDefault();
                //实体层
                ...
                {
                    BuildArea(areaName);
                    BuildController(areaName, aTable, (keyField == null ? "Id" : keyField.Name));
                }
                //视图
                if (buildTypeList.Exists(x => x.ToLower() == "view"))
                {
                    BuildArea(areaName);
                    BuildView(tableFieldInfo, areaName, aTable, (keyField == null ? "Id" : keyField.Name));
                }
            });
        }
...
{{
    ...
    {{
        ...
        /// <param name="areaName">区域名</param>
        /// <param name="entityName">实体名</param>
        private void BuildController(string areaName, string entityName, string keyField)
        {
...
{{
    ...
    {{
        ...
        {{
            var theData = id == 0 ? new {entityName}() : {varBusiness}.GetTheData(id);
        ...
        /// <param name="areaName">区域名</param>
        /// <param name="entityName">实体名</param>
        private void BuildView(List<TableInfo> tableInfoList, string areaName, string entityName, string keyField)
        {
            //生成Index页面
            ...
            tableInfoList.Where(x => x.Name != keyField).ForEach((aField, index) =>
                {
                    //搜索的下拉选项
  ...
  <system.webServer>
    <handlers>
      ...
      <add name=""BlockViewHandler"" path=""*"" verb=""*"" preCondition=""integratedMode"" type=""System.Web.HttpNotFoundHandler"" />
        ...
        /// <param name="linkId">数据库连接Id</param>
        /// <returns></returns>
        private DbHelper GetTheDbHelper(int linkId)
        {
        ...
        /// <param name="linkId">连接Id</param>
        /// <returns></returns>
        private Base_DatabaseLink GetTheLink(int linkId)
        {
